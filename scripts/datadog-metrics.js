...
async function send(){ const url=`https://api.${site}/api/v2/series`; try{ const res=await fetch(url,{ method:'POST', headers:{ 'Content-Type':'application/json','DD-API-KEY':apiKey }, body:JSON.stringify(payload)}); if(!res.ok){ console.error('Datadog metrics POST failed', res.status, await res.text()); } else { console.log('Datadog metrics sent:', JSON.stringify(payload,null,2)); } }catch(e){ console.error('Error sending metrics:', e.message);} }
send();
